...
y_test.value_counts().plot(kind="bar", ax=ax, color = "dodgerblue", label = "Train" )
plt.legend()
plt.show()
...
feature_set_2 = list(set(basic_features + square_features + pca_feature))
...
# Initialize an empty DataFrame for storing scores
score_df = pd.DataFrame(columns=["model", "feature_set", "accuracy"])
...
# --------------------------------------------------------------
# Ensure the correct feature selection and shapes
print(X_test[feature_set_4].shape, y_test.shape)
...
# Check if the number of predictions match the test labels
print(class_test_y.shape, y_test.shape)

# Compute accuracy
accuracy = accuracy_score(y_test, class_test_y)

# Extract the classes (ensure proper dimension)
...
# Adding values to the confusion matrix
thresh = cm.max() / 2.0
for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
    plt.text(
        j, i, format(cm[i, j]),
