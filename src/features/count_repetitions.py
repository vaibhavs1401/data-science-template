...
# Load data
# --------------------------------------------------------------
df = pd.read_pickle("../../data/interim/01_data_processd.pkl")
df = df[df["label"] != "rest"]
...
column = "acc_y"
LowPass.low_pass_filter(bench_set, col = column,  
sampling_frequency=fs, cutoff_frequency=0.3, order = 10)[column + "_lowpass"].plot()
...
def count_reps(dataset, cutoff=0.4, order=10, column="acc_r"):
    ...
    # Plot the filtered data and the detected peaks
    fig, ax = plt.subplots()
    plt.plot(data[f"{column}_lowpass"], label="Filtered Signal")
    plt.plot(peaks.index, peaks[f"{column}_lowpass"], "o", color="red", label="Peaks")
    ax.set_ylabel(f"{column}_lowpass")
    
    # Get exercise and category info for the title
    exercise = dataset["label"].iloc[0].title()  # Corrected variable name
    category = dataset["category"].iloc[0].title()
    plt.title(f"{category} {exercise}: {len(peaks)} Reps")
    plt.legend()
    plt.show()

    # Return the number of detected peaks (reps)
    return len(peaks)
count_reps(ohp_set, cutoff = 0.4)


count_reps(bench_set, cutoff = 0.4)
count_reps(squat_set, cutoff = 0.5)
count_reps(row_set, cutoff = 0.65)
count_reps(dead_set, cutoff = 0.35, column = "gyr_r")
count_reps(ohp_set, cutoff = 0.4)
