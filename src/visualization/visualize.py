...
participant_df = df.query("label == 'dead'").sort_values("participant").reset_index()
...
participant_df = df.query("label == 'rest'").sort_values("participant").reset_index()
...
fig, ax = plt.subplots()
participant_df.groupby(["participant"])["gyr_x"].plot()
ax.set_ylabel("gyr_x")
ax.set_xlabel("samples")
plt.legend()
...
for label in labels:
    for participant in participants:
        ...
        if len(combined_plot_df) > 0:
            ...
            # Corrected file name with f-string
